...
export async function atualizarCategoria(req, res) {
  try {
    const { id } = req.params;
    const dados = req.body;
    const categoriaAtualizada = await categoriaService.atualizarCategoria(Number(id), dados);
...
export async function removerCategoria(req, res) {
  try {
    const { id } = req.params;
    const categoriaRemovida = await categoriaService.removerCategoria(Number(id));
